...
### 이메일 인증 타임아웃 오류
**증상**: "이메일 인증 네트워크 오류 timeout" 메시지

**원인**: 
1. `.env` 파일이 없거나 SMTP 설정이 누락됨
2. Gmail 앱 비밀번호가 설정되지 않음
3. 2단계 인증이 활성화되지 않음

**해결 방법**:
1. 프로젝트 루트에 `.env` 파일 생성
2. Gmail 계정에서 2단계 인증 활성화
3. 앱 비밀번호 생성 후 `.env`에 설정
4. 자세한 설정은 `SETUP_EMAIL.md` 참조

### 일반적인 오류들
- **"Internal server error"**: 로그 확인 후 환경 변수 설정 점검
- **"SMTP 인증 실패"**: Gmail 앱 비밀번호 재생성
- **"Database connection error"**: MySQL 서버 상태 및 DATABASE_URL 확인
